  ...
  return (
    <>
      <span
        ...
        onMouseEnter={(event) => setTooltip({ text, x: event.clientX, y: event.clientY })}
        onMouseMove={(event) => setTooltip((current) => current ? { ...current, x: event.clientX, y: event.clientY } : current)}
        onMouseLeave={() => setTooltip(null)}
      >
        <InfoIcon />
      </span>
      {tooltip && <div className="book-info-tooltip" style={{ left: tooltip.x, top: tooltip.y - 12 }}>{tooltip.text}</div>}
    </>
  );
};
